...
namespace AppExtras.Renderers
{
    public class FadeRenderer : IRenderer
    {
        ...
        public void DrawMask(View target, Canvas maskCanvas, Color maskColor, Point position, int radius, int width, int height, Animator animator)
        {
            // draw solid background
            maskCanvas.DrawColor(maskColor);
		
			var rectangle = new Rect(target.Left, position.Y - height/2, target.Right, position.Y + height / 2);
